...
def second_problem():
    second_problem = '''
    select takes.course_id,course.title,c.semester,c.year
    from course,takes,takes as c
    where(
        c.course_id=takes.course_id
        and
...
def forth_problem():
    forth_problem='''
    select distinct t.ID,student.name,course.title,t.grade
    from 
    (
        select *
        from takes
...
def fifth_problem():
    ...
    size=len(budget)
    mid_number=(budget[size//2][0]+budget[(size+1)//2][0])/2
